            ...
            <div ref={ref} className='relative mx-auto max-w-7xl pb-20'>
                {data.map((item, index) => (
                    <div key={index} className='flex justify-start pt-10 md:gap-10 md:pt-40'>
                        <div className='sticky top-40 z-20 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm'>
                            ...
                            <h3 className='hidden text-xl font-bold text-primary/90 md:block md:pl-20 md:text-5xl'>
                                {item.title}
                            </h3>
                        </div>
                ...
                >
                    <motion.div
                        ...
                        className='absolute inset-x-0 top-0 w-[2px] rounded-full bg-gradient-to-t from-blue-600 from-[0%] via-brand via-[10%] to-transparent'
                    />
                </div>
            </div>
        </div>
